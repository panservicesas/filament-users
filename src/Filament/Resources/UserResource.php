...
class UserResource extends Resource
{
    ...
    {
        return config('filament-users.resource.model', User::class);
    }
    ...
    private static function getFormSchema(Schema $schema): array
    {
        ...
        if (Utils::isFilamentShieldInstalled()) {
            $multipleRoles = config('filament-users.resource.roles.multiple', false);
            switch (config('filament-users.resource.roles.type', 'select')) {
                case 'select':
                ...
                case 'checkbox':
                    ...
                    break;
                default:
                    throw new \InvalidArgumentException('Invalid role type configuration');
            }
        }
        ...
        $fields[] = Forms\Components\Toggle::make('generate_password')
            ...
            ->afterStateUpdated(function (bool $state, Set $set) {
                $set('password', $state ? Utils::generateRandomPassword() : null);
            })
            ->live();
    ...
    private static function getBulkActions(): array
    {
        ...
        if (Utils::isFilamentShieldInstalled()) {
            $actions[] = BulkAction::make('assign_role')
                ->label(__('filament-users::filament-users.resource.assign_role'))
                ...
                ->action(function ($records, array $data) {
                    try {
                        ...
                        Notification::make()
                            ->title(__('filament-users::filament-users.resource.role_assigned'))
                            ->success()
                            ->send();
                        ...
                        Log::error($e->getMessage());
                        Notification::make()
                            ->title(__('filament-users::filament-users.resource.role_not_assigned'))
                            ->danger()
                            ->send();
                    }
                });
    ...
    public static function getRelations(): array
    {
        ...
        if (Utils::isFilamentAuthenticationLogInstalled()) {
            $relations[] = AuthenticationLogsRelationManager::class;
        }
